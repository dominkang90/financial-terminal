...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between items-center py-1.5 border-b border-terminal-border/70">
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-4">
      ...
      <div className="bg-terminal-panel border border-terminal-border rounded-xl px-3 py-1">
        {children}
      </div>
    </div>
  );
  ...
  return (
    <div className="p-4 overflow-y-auto font-mono">
      ...
      <Section title="밸류에이션">
        ...
        <Row label="52주 최고" value={data.week52_high != null ? `$${fmt(data.week52_high)}` : "확인 중"} />
        <Row label="52주 최저" value={data.week52_low != null ? `$${fmt(data.week52_low)}` : "확인 중"} />
        <Row label="베타" value={fmt(data.beta)} />
      </Section>

      <Section title="재무 지표">
        <Row label="매출" value={fmtB(data.total_revenue)} />
        <Row label="EPS (현재)" value={data.eps_trailing != null ? `$${fmt(data.eps_trailing)}` : "확인 중"} />
        <Row label="EPS (예상)" value={data.eps_forward != null ? `$${fmt(data.eps_forward)}` : "확인 중"} />
      ...
      <Section title="애널리스트 의견">
        {recKey && (
          <div className="flex items-center justify-between py-1.5 border-b border-terminal-border/70">
            ...
            {(["strongBuy", "buy", "hold", "underperform", "sell"] as const).map((k) => {
              ...
              return (
                <div key={k} className="flex items-center gap-2 mb-1">
                  ...
                  <div className="flex-1 bg-terminal-border rounded-full h-1.5 overflow-hidden">
                    <div
      ...
      <Section title="목표주가">
        <Row label="목표가 최고" value={data.target_high != null ? `$${fmt(data.target_high)}` : "확인 중"} />
        <Row label="목표가 평균" value={data.target_mean != null ? `$${fmt(data.target_mean)}` : "확인 중"} />
        <Row label="목표가 중간값" value={data.target_median != null ? `$${fmt(data.target_median)}` : "확인 중"} />
        <Row label="목표가 최저" value={data.target_low != null ? `$${fmt(data.target_low)}` : "확인 중"} />
      </Section>
